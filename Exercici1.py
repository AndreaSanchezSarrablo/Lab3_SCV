...
#Creating a HLS transport stream container with a current video in the computer
def ex1(input_video):
	#Read input video and resized to multiple resolutions required (original video resolution, 1280x720 and 640x360
	# Transcode the video to multiple bitrates for HLS packing
	# Creating HLS Playlists (m3u8)
	os.system('ffmpeg -i ' + str(input_video) + '.mp4 \
	-filter_complex \
	"[0:v]split=3[v1][v2][v3]; \
	[v1]copy[v1out]; [v2]scale=w=1260:h=720[v2out]; [v3]scale=w=640:h=360[v3out]" \
...
#Finally we call the function with the input video
ex1("1min_cuttedVideo")
